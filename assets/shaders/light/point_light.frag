...
//                f_color = vec4(1.0, 1.0, 1.0, 1.0);
        ...
        vec3 lightDir = normalize(push_constants.position.xyz - texture(u_frag_pos, uv).rgb);
        float diff = max(dot(norm, lightDir), 0.0);

        float light_distance = length(push_constants.position.xyz - texture(u_frag_pos, uv).xyz);
