...
fn main2() {
    ...
    let window = surface.window();
    RenderSystem::new(&instance, surface.clone());
}
...
fn main() {
    ...
    loop {
        ...
        if recreate_swapchain {
            ...
            // Because framebuffers contains an Arc on the old swapchain, we need to
            // recreate framebuffers as well.
            let (new_pipeline, new_framebuffers) = window_size_dependent_setup(device.clone(), &vs, &fs, &images, render_pass.clone());
        ...
        let mut command_buffer = AutoCommandBufferBuilder::primary_one_time_submit(device.clone(), queue.family()).unwrap()
            ...
            &DynamicState::none(),
            vec!(triangle1.vertex_buffer.clone()),
        ...
        &DynamicState::none(),
        vec!(triangle2.vertex_buffer.clone()),
