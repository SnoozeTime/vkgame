...
impl AmbientLightingSystem {
        ...
        R: RenderPassAbstract + Send + Sync + 'static,
    {
        ...
        let pipeline =
            AmbientLightingSystem::build_pipeline(queue.clone(), subpass, [1, 1], &vs, &fs);
    ...
    pub fn draw(&self, color_input: &GBufferComponent, color: [f32; 3]) -> AutoCommandBuffer {
    ...
    pub fn handle_event(&mut self, ev: &Event) {
        if let Event::ResourceEvent(ResourceEvent::ResourceReloaded(ref path)) = ev {
            if (*path).ends_with("quad.vert") || (*path).ends_with("ambient_light.frag") {
                if let Err(err) = self
                    .vs
